...
class ReadArranger(object):
    def __init__(self, read_records: Tuple):
        '''
        Class to rearrange read records passed to it and output corrected read record. Use case scito ADT - move well
        bc to cell bc read :param read_records: Tuple[ReadRecord]. Tuple of read records
        '''
...
class FQReadArrangerAdtAtac(ReadArranger):
    '''
    Class contains only read2 and read3 - per technology description. self.read_records[0] is read2,
    self.read_records[1] is read3 :return ReadRecord. rearranged read2 (Cell barcode 16nt) + 5nt of well barcode
    '''
