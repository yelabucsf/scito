...
class BitRecordError(Exception):
    '''Handles improper use of BitRecord'''
...
class BitRecord(object):
    ...
    @staticmethod
    def get_seq_fragment(read_record, start: int = 0, seq_length: int = 0) -> str:
        '''
        ...
        if seq_length > len(read_record.seq):
            raise BitRecordError(
                f"BitRecord.get_seq_fragment(): sequence {read_record.read_id} is truncated. Aborting!")
        if seq_length == 0:
            seq_length = len(seq)
